...
# 2. ESTILOS VISUALES (REJILLA RESPONSIVA MEJORADA)
st.markdown("""
    <style>
    ...
    .vod-grid {
        display: grid;
        /* PC: Columnas automáticas de mínimo 140px */
        grid-template-columns: repeat(auto-fill, minmax(140px, 1fr));
        gap: 12px;
        padding-bottom: 20px;
    }
    
    /* MÓVIL: Forzar 3 columnas exactas */
    @media (max-width: 600px) {
        ...
        .vod-cat { font-size: 9px !important; }
    }
    ...
    .vod-card {
        ...
        border-radius: 6px;
        overflow: hidden; /* Recorta la imagen en las esquinas */
        border: 1px solid #333;
    ...
    .vod-img {
        width: 100%;
        height: 100%;
        object-fit: cover; /* Llena todo el espacio sin bordes negros */
        display: block;
    }
    ...
    with c2:
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("INICIAR SESIÓN"):
                if not st.session_state.user_ip_cached:
                    st.error("⚠️ Espera a que se detecte la IP.")
                    st.stop()
                    ...
                    with st.spinner("⏳ Conectando..."):
                        try:
                            ...
                            if res.status_code == 200:
                                ...
                                except: st.error("❌ Error de datos del servidor.")
...
# ==============================================================================
#  PANTALLA 3: DASHBOARD (GRID HTML)
# ==============================================================================
...
# CARGA DATOS
def fetch_data_and_cats(act_cont, act_cats):
    try:
        headers = {"User-Agent": "Mozilla/5.0"}
...
if mode == 'live' and not st.session_state.data_live:
    with st.spinner("Cargando Canales..."): st.session_state.data_live = fetch_data_and_cats('get_live_streams', 'get_live_categories')
elif mode == 'vod' and not st.session_state.data_vod:
    with st.spinner("Cargando Películas..."): st.session_state.data_vod = fetch_data_and_cats('get_vod_streams', 'get_vod_categories')
elif mode == 'series' and not st.session_state.data_series:
    with st.spinner("Cargando Series..."): st.session_state.data_series = fetch_data_and_cats('get_series', 'get_series_categories')
...
if mode == 'live':
    ...
    for item in filtered[:100]:
        ...
        <div class="channel-row">
            ...
            <div style="flex-grow:1;">
                <div style="font-size:12px; color:#aaa; font-weight:600;">{cat_name.upper()}</div>
    ...
    # GRID HTML RESPONSIVO (MOVIL 3 COLUMNAS / PC AUTO)
    limit = 60
    ...
    for item in view_items:
        ...
        title = item.get('name')
        # Escapamos comillas dobles en el titulo para no romper el HTML
        ...
        <div class="vod-card">
            ...
            <div class="vod-info">
                <div class="vod-title" title="{title_safe}">{title}</div>
