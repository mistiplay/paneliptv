...
# 2. ESTILOS VISUALES (SOLUCIÓN MÓVIL 3 COLUMNAS Y SIN MÁRGENES)
st.markdown("""
    <style>
    ...
    /* --- REJILLA HTML (SOLUCIÓN DEFINITIVA DE VISUALIZACIÓN) --- */
    .vod-grid {
        display: grid;
        /* PC: Columnas pequeñas (mínimo 120px) para que entren muchas */
        grid-template-columns: repeat(auto-fill, minmax(130px, 1fr));
        gap: 8px; /* Espacio pequeño entre tarjetas */
        padding-bottom: 20px;
    }
    
    /* MÓVIL: Forzar exactamente 3 columnas */
    @media (max-width: 600px) {
        .vod-grid {
            grid-template-columns: repeat(3, 1fr) !important;
            gap: 5px !important;
        }
        /* Ajustar texto para que quepa en movil */
    ...
    /* CONTENEDOR IMAGEN EXACTO */
    .vod-img-container {
        width: 100%;
        /* Esto mantiene la proporción vertical poster 2:3 */
        aspect-ratio: 2/3; 
        position: relative;
    }
    .vod-img {
        width: 100%;
        height: 100%;
        object-fit: cover; /* CLAVE: Esto elimina las franjas negras estirando la imagen */
        display: block;
    }
    ...
    .vod-info {
        ...
        /* Altura fija pequeña para el texto */
        height: 45px; 
        display: flex;
        flex-direction: column;
...
# ==============================================================================
#  PANTALLA 3: DASHBOARD (VISUAL ARREGLADO)
# ==============================================================================
...
if mode == 'live':
    ...
    for item in filtered[:100]:
        ...
        <div class="channel-row">
            ...
            <div style="flex-grow:1;">
                <div style="font-size:12px; color:#aaa; font-weight:600; text-transform:uppercase;">{cat_name}</div>
    ...
    # GRID HTML LIMPIO (SOLUCIÓN ERROR VISUAL)
    limit = 60
    view_items = filtered[:limit]
    
    # Creamos un solo bloque HTML para la rejilla
    grid_html = '<div class="vod-grid">'
    
    for item in view_items:
        img = proxy_img(item.get('stream_icon') or item.get('cover'))
        title = item.get('name').replace('"', '') # Limpiar comillas
    ...
    # Renderizamos TODO el grid de una vez para evitar errores
